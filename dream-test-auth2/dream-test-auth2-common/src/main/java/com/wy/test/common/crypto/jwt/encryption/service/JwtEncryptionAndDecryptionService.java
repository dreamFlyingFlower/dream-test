package com.wy.test.common.crypto.jwt.encryption.service;
...
public interface JwtEncryptionAndDecryptionService {
	...
	/**
	 * Encrypts the JWT in place with the default encrypter. If an arbitrary payload
	 * is used, then pass in a JWEObject. Otherwise, if JWT claims are the payload,
	 * then use the JWEObject subclass EncryptedJWT instead.
	 * 
	 * @param jwt
	 */
	...
	/**
	 * Decrypts the JWT in place with the default decrypter. If an arbitrary payload
	 * is used, then pass in a JWEObject. Otherwise, if JWT claims are the payload,
	 * then use the JWEObject subclass EncryptedJWT instead.
	 * 
	 * @param jwt
