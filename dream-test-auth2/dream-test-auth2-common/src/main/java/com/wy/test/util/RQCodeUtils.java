...
public class RQCodeUtils {
	...
	public static void write2OutputStream(OutputStream stream, String rqCodeText, String format, int width,
			int height) {
		try {
			BitMatrix byteMatrix = genRQCode(rqCodeText, width, height);
	...
	public static BitMatrix genRQCode(String rqCodeText, int width, int height) {
		...
		try {
			return new MultiFormatWriter().encode(rqCodeText, BarcodeFormat.QR_CODE, width, height);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
