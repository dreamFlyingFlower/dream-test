...
/**
 * This {@link PasswordEncoder} is provided for legacy purposes only and is not
 * considered secure.
 *
 * A version of {@link PasswordEncoder} which supports Ldap SHA and SSHA
 * (salted-SHA) encodings. The values are base-64 encoded and have the label
 * "{SHA}" (or "{SSHA}") prepended to the encoded hash. These can be made
 * lower-case in the encoded password, if required, by setting the
 * <tt>forceLowerCasePrefix</tt> property to true.
 *
 * Also supports plain text passwords, so can safely be used in cases when both
 * encoded and non-encoded passwords are in use or when a null implementation is
 * required.
 *
 * @author Luke Taylor deprecated Digest based password encoding is not
 *         considered secure. Instead use an adaptive one way function like
 *         BCryptPasswordEncoder, Pbkdf2PasswordEncoder, or
...
public class LdapShaPasswordEncoder implements PasswordEncoder {
	...
	/**
	 * Calculates the hash of password (and salt bytes, if supplied) and returns a
	 * base64 encoded concatenation of the hash and salt, prefixed with {SHA} (or
	 * {SSHA} if salt was used).
	 *
	...
	/**
	 * Checks the validity of an unencoded password against an encoded one in the
	 * form "{SSHA}sQuQF8vj8Eg2Y1hPdh3bkQhCKQBgjhQI".
	 *
