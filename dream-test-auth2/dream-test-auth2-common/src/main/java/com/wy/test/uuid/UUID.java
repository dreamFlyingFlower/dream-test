...
/**
 * Immutable representation of a Universally Unique Identifier (UUID), also
 * known (less presumptuously) as a Globally Unique Identifier (GUID). These
 * identifiers can be generated in a distributed fashion without central
 * coordination, and are reasonably small (128 bits), making them suitable for a
...
public final class UUID implements Serializable {
	...
	public static final UUID nil =
			new UUID((int) 0, (short) 0, (short) 0, (byte) 0, (byte) 0, new byte[] { 0, 0, 0, 0, 0, 0 });
	 ...
	 */
	public UUID(int time_low, short time_mid, short time_hi_and_version, byte clock_seq_low,
			byte clock_seq_hi_and_reserved, byte[] node) throws NullPointerException, IllegalArgumentException {
		if (node == null)
	 ...
	 * @throws IllegalArgumentException if s.length() != 36 or any expected hyphens
	 *         are missing
	...
	/**
	 * Reads the 16 bytes of a UUID from the specified DataInput source.
	 *
	...
	/**
	 * Writes the 16 data bytes of this UUID to the specified DataOutput interface.
	 *
	...
	public boolean equals(Object obj) {
		...
		if (hash_code != 0 && other.hash_code != 0 && hash_code != other.hash_code)
			return false;

		return time_low == other.time_low && time_mid == other.time_mid
				&& time_hi_and_version == other.time_hi_and_version && clock_seq_low == other.clock_seq_low
				&& clock_seq_hi_and_reserved == other.clock_seq_hi_and_reserved && Arrays.equals(node, other.node);
	}
	...
	/**
	 * Returns an array of 16 bytes containing the binary representation of this
	 * UUID.
	 */
	...
	public static UUID generate() {
		if (default_generator == null)
			default_generator =
					new TimestampUUIDGenerator(UUIDRandomness.randomClockSequence(), NodeIDGetter.getNodeID());

		return default_generator.nextUUID();
	}

	/**
	 * Creates a UUID based on a hash of a namespace designator UUID and a name.
	 */
	public static UUID fromName(UUID namespace, String name) {
		try {
			...
			uuid.clock_seq_hi_and_reserved =
					(byte) ((uuid.clock_seq_hi_and_reserved & ~VARIANT_DCE_MASK) | VARIANT_DCE);
