...
public class BeanConvert {
	...
	public static <T> Map<String, Object> bean2Map(T bean) {
		...
		for (int i = 0; i < flds.length; i++) {
			...
			if (BeanUtil.isGetProperty(bean.getClass(), fieldName)) {
				...
				LogFactory.getLog(BeanConvert.class).debug("bean2Map() field " + (i + 1) + " : " + fieldName + " = "
						+ value + " type : " + flds[i].getType());
			}
		}
		LogFactory.getLog(BeanConvert.class).debug("bean2Map() *******************************************");
		return mapBean;
	...
	public static <T> Object map2Bean(T bean, HashMap<?, ?> valueMap) {
		...
		while (fieldit.hasNext()) {
			...
			LogFactory.getLog(BeanConvert.class).debug(
					"map2Bean() field " + (i++) + " : " + fieldName + " = " + fillValue + " type : " + fieldType);
