...
public class MethodInvoke {
	...
	public static Object invokeMethod(Object bean, String methodName, Object[] args) throws Exception {
		...
		for (int i = 0, j = args.length; i < j; i++) {
			...
			// LogFactory.getLog(MethodInvoke.class).debug("invokeMethod args : "+args[i]+"
			// argsClass:"+argsClass[i]);
		}

		Method method = beanClass.getMethod(methodName, argsClass);
		// LogFactory.getLog(MethodInvoke.class).debug("invokeMethod
	...
	public static Object invokeStaticMethod(Class<?> beanClass, String methodName, Object[] args) throws Exception {
		...
		for (int i = 0, j = args.length; i < j; i++) {
			argsClass[i] = args[i].getClass();
			// LogFactory.getLog(MethodInvoke.class).debug("invokeStaticMethod args :
			// "+args[i]+" argsClass:"+argsClass[i]);
		}

		Method method = beanClass.getMethod(methodName, argsClass);
		// LogFactory.getLog(MethodInvoke.class).debug("invokeStaticMethod
		// methodName:"+methodName);
