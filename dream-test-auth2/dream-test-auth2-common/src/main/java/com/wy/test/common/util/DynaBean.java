...
public class DynaBean {
	...
	public void displayValues() {
		...
		while (beanMapit.hasNext()) {
			...
			LogFactory.getLog(DynaBean.class)
					.debug("displayValues() Field " + (i++) + " " + fieldName + " : " + beanMap.get(fieldName));
		}
		LogFactory.getLog(DynaBean.class).debug("displayValues() *******************************************");
	}
	...
	public <T> Object createBean(T cls) throws NoSuchMethodException, SecurityException, InstantiationException,
			IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		@SuppressWarnings("unchecked")
		T bean = ReflectHelper.newInstance((Class<T>) cls.getClass());
	...
	public DynaBean mapToDynaBean(Map<String, Object> map) {
		...
		if (map.getClass().getName() == "java.util.HashMap") {
			...
			while (mapIt.hasNext()) {
				...
				LogFactory.getLog(DynaBean.class)
						.debug("mapToDynaBean() Field " + (i++) + " " + fieldName + " : " + beanMap.get(fieldName));
			}
		}
		return this;
	}
