...
public final class X509V3CertGen {
	...
	public static X509Certificate genV3Certificate(String issuerName, String subjectName, Date notBefore, Date notAfter,
		...
		X509v3CertificateBuilder certBuilder =
				new X509v3CertificateBuilder(x500Name, serial, notBefore, notAfter, subject, subjectPublicKeyInfo);

		ContentSigner sigGen = new JcaContentSignerBuilder("SHA1withRSA").setProvider("BC").build(privateKey);
		// certBuilder.addExtension(X509Extensions.BasicConstraints, true, new
		// BasicConstraints(false));
		// certBuilder.addExtension(X509Extensions.KeyUsage, true, new
		// KeyUsage(KeyUsage.digitalSignature| KeyUsage.keyEncipherment));
		// certBuilder.addExtension(X509Extensions.ExtendedKeyUsage, true, new
		// ExtendedKeyUsage(KeyPurposeId.id_kp_serverAuth));
		// certBuilder.addExtension(X509Extensions.SubjectAlternativeName, false, new
		// GeneralNames(new GeneralName(GeneralName.rfc822Name, "connsec@163.com")));
	...
	public static KeyPair genRSAKeyPair() throws Exception {
		return KeyPairHelpers.genKeyPair(KeyPairType.RSA, "BC");
	}
}
