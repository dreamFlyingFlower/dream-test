...
public class Cas20AuthorizeEndpoint extends CasBaseAuthorizeEndpoint {
	 ...
	 * @param renew
	 * @param format
	 * @return 2.5. /serviceValidate [CAS 2.0] /serviceValidate checks the validity of a service ticket and returns an
	 *         XML-fragment response. /serviceValidate MUST also generate and issue proxy-granting tickets when
	 *         requested. /serviceValidate MUST NOT return a successful authentication if it receives a proxy ticket. It
	 *         is RECOMMENDED that if /serviceValidate receives a proxy ticket, the error message in the XML response
	 *         SHOULD explain that validation failed because a proxy ticket was passed to /serviceValidate.
	 * 
	 * 
	 *         2.5.1. parameters The following HTTP request parameters MAY be specified to /serviceValidate. They are
	 *         case sensitive and MUST all be handled by /serviceValidate.
	 * 
	 ...
	 * 
	 *         pgtUrl [OPTIONAL] - the URL of the proxy callback. Discussed in Section 2.5.4. As a HTTP request
	 *         parameter, the ��pgtUrl�� value MUST be URL-encoded as described in Section 2.2 of RFC 1738 [4].
	 * 
	 ...
	 * 
	 *         format [OPTIONAL] - if this parameter is set, ticket validation response MUST be produced based on the
	 *         parameter value. Supported values are XML and JSON. If this parameter is not set, the default XML format
	 *         will be used. If the parameter value is not supported by the CAS server, an error code MUST be returned
	 *         as is described in section 2.5.3.
	 * 
	 * 
	 *         2.5.2. response /serviceValidate will return an XML-formatted CAS serviceResponse as described in the XML
	 *         schema in Appendix A. Below are example responses:
	 * 
	 *         On ticket validation success: <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:authenticationSuccess> <cas:user>username</cas:user>
	 *         <cas:proxyGrantingTicket>PGTIOU-84678-8a9d...</cas:proxyGrantingTicket> </cas:authenticationSuccess>
	 *         </cas:serviceResponse>
	 * 
	 *         { "serviceResponse" : { "authenticationSuccess" : { "user" : "username", "proxyGrantingTicket" :
	 *         "PGTIOU-84678-8a9d..." } } } On ticket validation failure:
	 *         <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:authenticationFailure code="INVALID_TICKET"> Ticket ST-1856339-aA5Yuvrxzpv8Tau1cYQ7 not recognized
	 *         </cas:authenticationFailure> </cas:serviceResponse>
	 * 
	 *         { "serviceResponse" : { "authenticationFailure" : { "code" : "INVALID_TICKET", "description" : "Ticket
	 *         ST-1856339-aA5Yuvrxzpv8Tau1cYQ7 not recognized" } } }
	 * 
	 *         Example response with custom attributes <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:authenticationSuccess> <cas:user>username</cas:user> <cas:attributes>
	 *         <cas:firstname>John</cas:firstname> <cas:lastname>Doe</cas:lastname> <cas:title>Mr.</cas:title>
	 *         <cas:email>jdoe@example.org</cas:email> <cas:affiliation>staff</cas:affiliation>
	 *         <cas:affiliation>faculty</cas:affiliation> </cas:attributes>
	 *         <cas:proxyGrantingTicket>PGTIOU-84678-8a9d...</cas:proxyGrantingTicket> </cas:authenticationSuccess>
	 *         </cas:serviceResponse>
	 * 
	 *         { "serviceResponse" : { "authenticationSuccess" : { "user" : "username", "proxyGrantingTicket" :
	 *         "PGTIOU-84678-8a9d...", "proxies" : [ "https://proxy1/pgtUrl", "https://proxy2/pgtUrl" ], "attributes" :
	 *         { "firstName" : "John", "affiliation" : [ "staff", "faculty" ], "title" : "Mr.", "email" :
	 *         "jdoe@example.orgmailto:jdoe@example.org", "lastname" : "Doe" } } } } 2.5.3. error codes The following
	 *         values MAY be used as the ��code�� attribute of authentication failure responses. The following is the
	 ...
	 * 
	 *         INVALID_PROXY_CALLBACK - The proxy callback specified is invalid. The credentials specified for proxy
	 *         authentication do not meet the security requirements
	 * 
	 *         INVALID_TICKET - the ticket provided was not valid, or the ticket did not come from an initial login and
	 *         renew was set on validation. The body of the \<cas:authenticationFailure\> block of the XML response
	...
	@GetMapping(value = CasConstants.ENDPOINT.ENDPOINT_SERVICE_VALIDATE, produces = MediaType.APPLICATION_XML_VALUE)
	@ResponseBody
	 ...
	 * @param pgtUrl
	 * @param renew
	 * @return 2.6. /proxyValidate [CAS 2.0] /proxyValidate MUST perform the same validation tasks as /serviceValidate
	 *         and additionally validate proxy tickets. /proxyValidate MUST be capable of validating both service
	 *         tickets and proxy tickets. See Section 2.5.4 for details.
	 * 
	 * 
	 *         2.6.1. parameters /proxyValidate has the same parameter requirements as /serviceValidate. See Section
	 *         2.5.1.
	 * 
	 * 
	 *         2.6.2. response /proxyValidate will return an XML-formatted CAS serviceResponse as described in the XML
	 *         schema in Appendix A. Below are example responses:
	 * 
	 *         Response on ticket validation success: <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:authenticationSuccess> <cas:user>username</cas:user>
	 *         <cas:proxyGrantingTicket>PGTIOU-84678-8a9d...</cas:proxyGrantingTicket> <cas:proxies>
	 *         <cas:proxy>https://proxy2/pgtUrl</cas:proxy> <cas:proxy>https://proxy1/pgtUrl</cas:proxy> </cas:proxies>
	 *         </cas:authenticationSuccess> </cas:serviceResponse>
	 * 
	 *         { "serviceResponse" : { "authenticationSuccess" : { "user" : "username", "proxyGrantingTicket" :
	 *         "PGTIOU-84678-8a9d...", "proxies" : [ "https://proxy1/pgtUrl", "https://proxy2/pgtUrl" ] } } }
	 * 
	 *         Note: when authentication has proceeded through multiple proxies, the order in which the proxies were
	 *         traversed MUST be reflected in the <cas:proxies> block. The most recently-visited proxy MUST be the first
	 *         proxy listed, and all the other proxies MUST be shifted down as new proxies are added. In the above
	 *         example, the service identified by <https://proxy1/pgtUrl> was visited first, and that service proxied
	 *         authentication to the service identified by <https://proxy2/pgtUrl>.
	 * 
	 *         Response on ticket validation failure:
	 * 
	 *         <cas:serviceResponse xmlns:cas='http://www.yale.edu/tp/cas'>
	 *         <cas:authenticationFailure code="INVALID_TICKET"> ticket PT-1856376-1HMgO86Z2ZKeByc5XdYD not recognized
	 *         </cas:authenticationFailure> </cas:serviceResponse>
	 * 
	 *         { "serviceResponse" : { "authenticationFailure" : { "code" : "INVALID_TICKET", "description" : "Ticket
	 *         PT-1856339-aA5Yuvrxzpv8Tau1cYQ7 not recognized" } } }
	 */

	@Operation(summary = "CAS 2.0 ticket代理验证接口", description = "通过ticket获取当前登录用户信息", method = "POST")
	@GetMapping(value = CasConstants.ENDPOINT.ENDPOINT_PROXY_VALIDATE, produces = MediaType.APPLICATION_XML_VALUE)
	@ResponseBody
	 ...
	 * @param pgt
	 * @param targetService
	 * @return 2.7. /proxy [CAS 2.0] /proxy provides proxy tickets to services that have acquired proxy-granting tickets
	 *         and will be proxying authentication to back-end services.
	 * 
	 * 
	 *         2.7.1. parameters The following HTTP request parameters MUST be specified to /proxy. They are both
	 *         case-sensitive.
	 * 
	 *         pgt [REQUIRED] - the proxy-granting ticket acquired by the service during service ticket or proxy ticket
	 *         validation. targetService [REQUIRED] - the service identifier of the back-end service. Note that not all
	 *         back-end services are web services so this service identifier will not always be an URL. However, the
	 *         service identifier specified here MUST match the service parameter specified to /proxyValidate upon
	 *         validation of the proxy ticket.
	 * 
	 *         2.7.2. response /proxy will return an XML-formatted CAS serviceResponse document as described in the XML
	 *         schema in Appendix A. Below are example responses:
	 * 
	 *         Response on request success: <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:proxySuccess> <cas:proxyTicket>PT-1856392-b98xZrQN4p90ASrw96c8</cas:proxyTicket> </cas:proxySuccess>
	 *         </cas:serviceResponse>
	 * 
	 *         Response on request failure: <cas:serviceResponse xmlns:cas="http://www.yale.edu/tp/cas">
	 *         <cas:proxyFailure code="INVALID_REQUEST"> 'pgt' and 'targetService' parameters are both required
	 *         </cas:proxyFailure> </cas:serviceResponse> { "serviceResponse" : { "authenticationFailure" : { "code" :
	 *         "INVALID_REQUEST", "description" : "'pgt' and 'targetService' parameters are both required" } } }
	 * 
	 * 
	 *         2.7.3. error codes The following values MAY be used as the code attribute of authentication failure
	 *         responses. The following is the minimum set of error codes that all CAS servers MUST implement.
	 *         Implementations MAY include others.
	 * 
	 *         INVALID_REQUEST - not all of the required request parameters were present
	 * 
	 ...
	 *         <cas:authenticationFailure> block of the XML response.
	 */
	@GetMapping(value = CasConstants.ENDPOINT.ENDPOINT_PROXY, produces = MediaType.APPLICATION_XML_VALUE)
	@ResponseBody
	public String proxy(HttpServletRequest request, HttpServletResponse response,
