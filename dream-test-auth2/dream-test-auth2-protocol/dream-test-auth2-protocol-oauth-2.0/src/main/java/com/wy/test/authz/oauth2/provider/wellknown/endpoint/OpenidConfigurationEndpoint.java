package com.wy.test.authz.oauth2.provider.wellknown.endpoint;
...
import com.wy.test.authz.oauth2.provider.wellknown.OpenidConfiguration;
...
public class OpenidConfigurationEndpoint extends AbstractEndpoint {
	...
	@Operation(summary = "OpenID Connect metadata 元数据接口", description = "参数client_id", method = "GET,POST")
	@RequestMapping(value = { OAuth2Constants.ENDPOINT.ENDPOINT_BASE + "/{instId}/.well-known/openid-configuration" },
			produces = "application/json", method = { RequestMethod.POST, RequestMethod.GET })
	@ResponseBody
	public String configurationMetadata(HttpServletRequest request, HttpServletResponse response,
		...
		if (clientDetails != null) {
			...
			openidConfig.setJwks_uri(baseUrl + OAuth2Constants.ENDPOINT.ENDPOINT_BASE + "/jwks?client_id="
