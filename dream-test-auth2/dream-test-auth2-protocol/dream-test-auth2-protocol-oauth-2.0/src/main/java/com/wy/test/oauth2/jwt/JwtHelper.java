package com.wy.test.oauth2.jwt;
...
class JwtImpl implements Jwt {
	...
	JwtImpl(JwtHeader header, byte[] content, byte[] crypto) {
		...
		this.crypto = crypto;
		claims = Codecs.utf8Decode(content);
	}
	...
	private byte[] signingInput() {
		return Codecs.concat(Codecs.b64UrlEncode(header.bytes()), JwtHelper.PERIOD, Codecs.b64UrlEncode(content));
	}
	...
	@Override
	public byte[] bytes() {
		return Codecs.concat(Codecs.b64UrlEncode(header.bytes()), JwtHelper.PERIOD, Codecs.b64UrlEncode(content), JwtHelper.PERIOD,
				Codecs.b64UrlEncode(crypto));
	}

	@Override
	public String getClaims() {
		return Codecs.utf8Decode(content);
	}
