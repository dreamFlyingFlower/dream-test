package com.wy.test.protocol.saml2.authz.saml20.metadata;
...
public class MetadataGenerator {
	...
	public void samlmtest() {
		try {
			...
			TrustResolver trustResolver = new TrustResolver(trustKeyStore, "idp", keyStoreLoader.getKeystorePassword(),
					issueInstantRule, messageReplayRule, "POST");
	...
	public ContactPerson getContactPerson(String companyName, String givenName, String surName, String emailAddress,
			String telephoneNumber, ContactPersonTypeEnumeration contactPersonType) {
	...
	public SingleSignOnService getSingleSignOnService(String location, String binding) {
		SingleSignOnService singleSignOnService =
				(SingleSignOnService) buildXMLObject(SingleSignOnService.DEFAULT_ELEMENT_NAME);
		if (binding == null) {
	...
	public SingleLogoutService getSingleLogoutService(String location, String binding) {
		SingleLogoutService singleLogoutService =
				(SingleLogoutService) buildXMLObject(SingleLogoutService.DEFAULT_ELEMENT_NAME);
		if (binding == null) {
	...
	public static XMLObject unmarshallElement(Document doc) {
		try {
			...
			Unmarshaller unmarshaller =
					org.opensaml.xml.Configuration.getUnmarshallerFactory().getUnmarshaller(samlElement);
	...
	public static Element marshallerElement(XMLObject xmlObject) {
		...
		if (marshaller == null) {
			logger.error("Unable to locate marshaller for " + xmlObject.getElementQName()
